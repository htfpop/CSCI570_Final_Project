 ...
 *              October 4, 2020
 *
 * Updated by:  Christopher K. Leung
 *              November 21, 2022
 *
 */
...
public class writeXLSX {
    ...
    public writeXLSX(String s) throws IOException {
        this.fileName = s;
        wb = new XSSFWorkbook();
        sheet = wb.createSheet("Sequence Alignment Algorithms");
    }
     ...
     */
    public void writeLong(int rowNum, int columnNum, long algorithmTime){
        if(sheet.getRow(rowNum) == null)
            row = sheet.createRow(rowNum);
        row = sheet.getRow(rowNum);
        cell = row.createCell(columnNum);
     ...
     * @param rowNum - row number to access
     * @param columnNum - column number to create a cell in specified row
     * @param algorithmTime - writes a double integer to excel cell
     */
    public void writeDouble(int rowNum, int columnNum, double algorithmTime){
        if(sheet.getRow(rowNum) == null)
            row = sheet.createRow(rowNum);
        row = sheet.getRow(rowNum);
        cell = row.createCell(columnNum);
     ...
     * @param i - integer
     */
    public void writeInt(int rowNum, int columnNum, int i){
        if(sheet.getRow(rowNum) == null)
            row = sheet.createRow(rowNum);
        row = sheet.getRow(rowNum);
        cell = row.createCell(columnNum);
     ...
     * @param rowNum - row number to access
     * @param columnNum - column number to create a cell in specified row
     * @param s - writes a string to cell
     */
    public void writeString(int rowNum, int columnNum, String s){
        if(sheet.getRow(rowNum) == null)
            row = sheet.createRow(rowNum);
        row = sheet.getRow(rowNum);
        cell = row.createCell(columnNum);
